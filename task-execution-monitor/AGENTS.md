...
- `server/app.js` starts the Koa server, serves `public/`, registers routes, and schedules task execution checks.
...
The server listens on `config.server.port` from the loaded configuration. Check the startup log for the actual URL, for example `http://localhost:3006`.
...
Tests use Node's built-in `node:test` runner. Before submitting changes, run `npm test` and manually verify the affected API or UI path. Useful endpoints include configuration routes under `/api/config` and metrics routes under `/api/metrics`.
