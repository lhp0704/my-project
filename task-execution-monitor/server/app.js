...
async function checkMetrics() {
  ...
  const tasks = config.monitor.tasks || [];
  const taskData = await grafanaService.fetchAllTasks(tasks);

  updateMetrics(taskData);

  const alerts = alertService.checkAlerts(taskData);
...
app.listen(PORT, () => {
  ...
  console.log('   - 监控任务:', (config.monitor.tasks || []).length, '个');
  console.log('   - 检查间隔:', config.monitor.checkInterval, 'ms');
  console.log('   - 默认成功数阈值:', config.monitor.defaultSuccessCountThreshold ?? 1);
  console.log('   - 默认成功率阈值:', config.monitor.defaultSuccessRateThreshold ?? 95, '%');
  console.log('\n');
  startMonitor();
});
